...
import raddy/types             ## nk_context, nk_user_font
...
when defined(raddyFixed):
  spec "raddyRender (raddyFixed path — fixed command buffer)":
    ...
    it "empty queue returns no overflow on fixed-buffer context":
      ...
      var nkFont: nk_user_font
      var ctx: nk_context
      const fixedBufSize = 4096
      var cmdBuf: array[fixedBufSize, byte]
      let ok = raddyCtxInit(addr ctx, addr nkFont, addr cmdBuf[0], nk_size(fixedBufSize))
