...
- **Open combo dropdown / open popup**: Nuklear pushes `nk_null_rect = {x:-8192, y:-8192, w:16384, h:16384}` so the dropdown can draw outside the parent window's bounds. After the Y-flip (`y' = H - y - h`), the result is `{x:-8192, y:H-8192, w:16384, h:16384}` — a huge rect with a large negative Y. raylib's `BeginScissorMode` clamps this to the drawable area, which is the correct behaviour (the popup is visible everywhere on screen). On close, Nuklear restores the parent's clip rect with another SCISSOR command.
